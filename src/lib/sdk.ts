...
export function findHypeAuthorityAddress(programId: PublicKey): PublicKey {
  const [hypeAuthority] = PublicKey.findProgramAddressSync(
    [HYPE_SEED],
    programId,
  );
...
export function findRootAccountAddress(
  programId: PublicKey,
  version: number,
): PublicKey {
...
export async function mint(args: TradeArgs): Promise<{
  ...
  if (args.nickname != undefined) {
    buf.write(
      args.nickname,
      48,
      Math.min(NicknameStringLength, args.nickname.length),
      "utf-8",
    );
  }
  ...
  buf.write(
    ...
    8,
    Math.min(NetworkStringLength, args.address.length),
    "utf-8",
  );
  const instruction = new TransactionInstruction({
    keys: [
      ...
      {
        pubkey: args.root.baseCrncyProgramAddress,
        isSigner: false,
        isWritable: true,
      },
      {
...
export function changeClientData(
  args: ChangeClientDataArgs,
): TransactionInstruction {
  const clientAccount = findClientAccountAddress(
    args.programId,
    args.wallet,
