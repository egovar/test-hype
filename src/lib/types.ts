...
export class NetworkRecord {
  ...
  unpack(buf: Buffer, offset: number) {
    ...
    this.descriptor = getStringFromBuffer(
      buf,
      offset + NetworkRecordOffsets.Descriptor,
      NetworkStringLength,
    );
    this.mask = getStringFromBuffer(
      buf,
...
export class RootAccount {
  ...
  update(buf: Buffer) {
    ...
    this.baseCrncyProgramAddress = readPk(
      buf,
    ...
    for (let i = 0; i < this.networksCount; ++i) {
      ...
      const network = new NetworkRecord();
...
export class TokenAccount {
  ...
  update(buf: Buffer, baseCrncyDecsFactor: number) {
    ...
    this.supply =
      ...
      baseCrncyDecsFactor;
    this.address = getStringFromBuffer(
      buf,
...
export class ClientAccount {
  ...
  update(buf: Buffer, baseCrncyDecsFactor: number) {
    ...
    this.nickname = getStringFromBuffer(
      buf,
      ClientAccountOffsets.Nickname,
      NicknameStringLength,
    );
    this.allTimeBaseCrncyVolume =
      Number(buf.readBigInt64LE(ClientAccountOffsets.AllTimeBaseCrncyVolume)) /
      baseCrncyDecsFactor;
    this.allTimeTokensVolume =
      Number(buf.readBigInt64LE(ClientAccountOffsets.AllTimeTokensVolume)) /
      baseCrncyDecsFactor;
    this.allTimeTradesCount = Number(
      buf.readBigInt64LE(ClientAccountOffsets.AllTimeTradesCount),
    );
